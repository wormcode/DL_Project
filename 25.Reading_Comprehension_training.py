...
X = tf.placeholder(tf.int32, [batch_size, content_length])   # data
Q = tf.placeholder(tf.int32, [batch_size, question_length])  # questions
A = tf.placeholder(tf.int32, [batch_size])                   # answers
...
def train_neural_attention():
	...
	with tf.Session() as sess:
		...
		for step in range(20000):
			...
			# save the model and calculate accuracy
			if step % 1000 == 0:
