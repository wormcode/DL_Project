...
# paths of wav files(  training data )
	...
	for (dirpath, dirnames, filenames) in os.walk(wav_path):
		for filename in filenames:
			if filename.endswith('.wav') or filename.endswith('.WAV'):
				...
				if os.stat(filename_path).st_size < 240000:  # remove some small files
					continue
...
# label read the label of the correspond wav file
...
#print(words[479]) #green
label_max_len = np.max([len(label) for label in labels_vector])
print('word counts of the longest sentence:', label_max_len)
...
print("the max length of wav:", wav_max_len)
