...
# if use mnist dataset，set MNIST=True
MNIST = False
...
if MNIST == True:
	...
	picture_list = []
	# not load all of the images at a time, to save memory overhead,load when needed
	for (dirpath, dirnames, filenames) in os.walk(picture_dir):
		for filename in filenames:
			if filename.endswith('.jpg'):
				picture_list.append(os.sep.join([dirpath, filename]))
 
	print("total number of images: ", len(picture_list))
